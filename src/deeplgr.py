# initial implementation from https://github.com/yoshall/DeepLGR
...
class DeepLGR(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.predictor == "td":  # tensor decomposition
            out = out.reshape(b, self.n_filters, -1).permute(
                0, 2, 1
            )  # [b, H*W, n_filters]
            region_param = torch.matmul(
                self.core, self.F
            )  # [16, 16, n_filters*out_channels]
            ...
            region_param = region_param.unsqueeze(0).repeat(
                b, 1, 1, 1
            )  # [b, n_f*out_c, H, W]
            region_param = region_param.reshape(
                b, -1, self.n_filters, self.height * self.width
            ).permute(0, 3, 2, 1)  # [b, H*W, n_f, 2]
            region_features = out.unsqueeze(3).repeat(
            ...
            region_param = torch.matmul(self.L, self.R).unsqueeze(
                0
            )  # [1, H*W, n_filter*2]
...
class GlobalNet(nn.Module):
    def __init__(
        self, features=64, out_features=64, sizes=(1, 2, 4, 8), height=128, width=128
    ):
        super().__init__()
        self.stages = []
        ...
        self.bottleneck = nn.Conv2d(
            features + features // 8 * 4, out_features, kernel_size=1
        )
        self.relu = nn.ReLU()
        ...
        for size in sizes:
            self.deconvs.append(
                SubPixelBlock(features // 8, upscale_factor=height // size)
            )
    ...
    def forward(self, x):
        priors = [
            upsample(stage(x)) for stage, upsample in zip(self.stages, self.deconvs)
        ]
        out = priors + [x]
