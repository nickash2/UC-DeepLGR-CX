import os
import h5py
import numpy as np
import pandas as pd
from datetime import datetime
import pickle


class TaxiBJPreprocessor:
    def __init__(self, data_dir="data", output_dir="data/processed"):
        """
        Initialize preprocessor for TaxiBJ dataset.
        
        Args:
            data_dir: Directory containing raw H5 files
            output_dir: Directory to save preprocessed data
        """
        self.data_dir = data_dir
        self.output_dir = output_dir
        os.makedirs(output_dir, exist_ok=True)
        
        # Timeslot filtering: 6am = slot 12, 11pm = slot 46 (exclusive)
        # With 30-min intervals: 0=00:00, 12=06:00, 46=23:00
        self.min_slot = 12  # 6am
        self.max_slot = 46  # 11pm (exclusive, so last slot is 22:30-23:00)
        
    def load_flow_data(self, years=["16"]):
        """
        Load inflow/outflow data from TaxiBJ H5 file(s).
        
        Args:
            years: List of year suffixes (e.g., ["13", "14", "15", "16"])
            
        Returns:
            data: Flow data array [n_timeslots, 2, 32, 32]
            dates: List of datetime strings
        """
        if isinstance(years, str):
            years = [years]
        
        all_data = []
        all_dates = []
        
        for year in years:
            filepath = os.path.join(self.data_dir, f"BJ{year}_M32x32_T30_InOut.h5")
            print(f"Loading flow data from {filepath}...")
            
            with h5py.File(filepath, "r") as f:
                data = f["data"][:]  # [n_timeslots, 2, 32, 32]
                dates = [d.decode() if isinstance(d, bytes) else d for d in f["date"][:]]
            
            print(f"  Loaded {len(dates)} timeslots from BJ{year}")
            all_data.append(data)
            all_dates.extend(dates)
        
        # Concatenate all years
        combined_data = np.concatenate(all_data, axis=0)
        print(f"Combined total: {len(all_dates)} timeslots, shape: {combined_data.shape}")
        
        return combined_data, all_dates
    
    def filter_timeslots(self, data, dates):
        """
        Filter timeslots to only include 6am-11pm (slots 12-45).
        
        Args:
            data: Flow data [n_timeslots, 2, 32, 32]
            dates: List of datetime strings
            
        Returns:
            filtered_data: Filtered flow data
            filtered_dates: Filtered dates
            valid_indices: Original indices of kept timeslots
        """
        print(f"Filtering timeslots to 6am-11pm range...")
        
        # Parse dates to extract time slot of day
        # Date format: "2015110101" = YYYYMMDDSS where SS is slot number (01-48)
        valid_indices = []
        for idx, date_str in enumerate(dates):
            # Extract slot number from last 2 digits
            # Date format: YYYYMMDDSS where SS is slot 01-48 (not 00-47)
            slot_str = date_str[-2:]  # Last 2 characters
            slot_of_day = int(slot_str) - 1  # Convert to 0-indexed (0-47)
            
            # Filter: keep slots 12-45 (6am to 11pm)
            # Slot 12 = 6:00am, Slot 45 = 10:30pm
            if self.min_slot <= slot_of_day < self.max_slot:
                valid_indices.append(idx)
        
        filtered_data = data[valid_indices]
        filtered_dates = [dates[i] for i in valid_indices]
        
        print(f"Kept {len(valid_indices)} timeslots after filtering")
        return filtered_data, filtered_dates, valid_indices
    
    def load_meteorology(self):
        """
        Load meteorology data: temperature, wind speed, weather one-hot.
        
        Returns:
            meteo_data: Dict with keys 'Temperature', 'WindSpeed', 'Weather'
            meteo_dates: List of datetime strings
        """
        filepath = os.path.join(self.data_dir, "BJ_Meteorology.h5")
        print(f"Loading meteorology data from {filepath}...")
        
        with h5py.File(filepath, "r") as f:
            temperature = f["Temperature"][:]
            wind_speed = f["WindSpeed"][:]
            weather = f["Weather"][:]  # One-hot encoded [n_timeslots, 17]
            dates = [d.decode() if isinstance(d, bytes) else d for d in f["date"][:]]
        
        meteo_data = {
            "Temperature": temperature,
            "WindSpeed": wind_speed,
            "Weather": weather
        }
        
        print(f"Loaded meteorology for {len(dates)} timeslots")
        print(f"  Temperature shape: {temperature.shape}")
        print(f"  WindSpeed shape: {wind_speed.shape}")
        print(f"  Weather shape: {weather.shape}")
        
        return meteo_data, dates
    
    def load_holidays(self):
        """
        Load holiday dates from BJ_Holiday.txt.
        
        Returns:
            holidays: Set of date strings in YYYYMMDD format
        """
        filepath = os.path.join(self.data_dir, "BJ_Holiday.txt")
        print(f"Loading holiday data from {filepath}...")
        
        holidays = set()
        with open(filepath, "r") as f:
            for line in f:
                date_str = line.strip()
                if date_str:
                    holidays.add(date_str)
        
        print(f"Loaded {len(holidays)} holiday dates")
        return holidays
    
    def align_external_features(self, flow_dates, meteo_data, meteo_dates, holidays):
        """
        Align meteorology and calendar features with flow data timestamps.
        
        Args:
            flow_dates: Flow data timestamps
            meteo_data: Dict of meteorology arrays
            meteo_dates: Meteorology timestamps
            holidays: Set of holiday date strings
            
        Returns:
            aligned_features: Array [n_timeslots, n_features]
                Features: [temperature, wind_speed, weather_one_hot (17), is_weekend, is_holiday]
        """
        print("Aligning external features with flow data...")
        
        # Create date lookup for meteorology
        meteo_dict = {date: idx for idx, date in enumerate(meteo_dates)}
        
        n_timeslots = len(flow_dates)
        # Features: temp(1) + wind(1) + weather(17) + weekend(1) + holiday(1) = 21
        aligned_features = np.zeros((n_timeslots, 21), dtype=np.float32)
        
        for i, date_str in enumerate(flow_dates):
            # Match meteorology data
            if date_str in meteo_dict:
                meteo_idx = meteo_dict[date_str]
                aligned_features[i, 0] = meteo_data["Temperature"][meteo_idx]
                aligned_features[i, 1] = meteo_data["WindSpeed"][meteo_idx]
                aligned_features[i, 2:19] = meteo_data["Weather"][meteo_idx]
            else:
                # Missing meteorology - use neutral values
                print(f"Warning: Missing meteorology for {date_str}")
                aligned_features[i, 0] = 0  # Will be normalized later
                aligned_features[i, 1] = 0
                aligned_features[i, 2] = 1  # Default to "Sunny" (index 0)
            
            # Calendar features
            # Extract date (YYYYMMDD) from date string (YYYYMMDDSS)
            date_only = date_str[:8]  # First 8 characters: YYYYMMDD
            dt = datetime.strptime(date_only, "%Y%m%d")
            is_weekend = 1.0 if dt.weekday() >= 5 else 0.0  # Sat=5, Sun=6
            is_holiday = 1.0 if date_only in holidays else 0.0
            
            aligned_features[i, 19] = is_weekend
            aligned_features[i, 20] = is_holiday
        
        print(f"Aligned features shape: {aligned_features.shape}")
        return aligned_features
    
    def normalize_data(self, flow_data, external_features):
        """
        Normalize flow data and continuous external features.
        
        Args:
            flow_data: Flow data [n_timeslots, 2, 32, 32]
            external_features: External features [n_timeslots, 21]
            
        Returns:
            normalized_flow: Normalized flow data
            normalized_features: Normalized external features
            stats: Dict of normalization statistics for denormalization
        """
        print("Normalizing data...")
        
        # Normalize flow data (min-max to [0, 1])
        flow_min = flow_data.min()
        flow_max = flow_data.max()
        normalized_flow = (flow_data - flow_min) / (flow_max - flow_min + 1e-8)
        
        # Normalize continuous features (temperature, wind speed)
        # Weather one-hot, weekend, holiday are already 0-1
        normalized_features = external_features.copy()
        
        temp_mean = external_features[:, 0].mean()
        temp_std = external_features[:, 0].std()
        normalized_features[:, 0] = (external_features[:, 0] - temp_mean) / (temp_std + 1e-8)
        
        wind_mean = external_features[:, 1].mean()
        wind_std = external_features[:, 1].std()
        normalized_features[:, 1] = (external_features[:, 1] - wind_mean) / (wind_std + 1e-8)
        
        stats = {
            "flow_min": float(flow_min),
            "flow_max": float(flow_max),
            "temp_mean": float(temp_mean),
            "temp_std": float(temp_std),
            "wind_mean": float(wind_mean),
            "wind_std": float(wind_std)
        }
        
        print(f"Flow data normalized: min={flow_min:.2f}, max={flow_max:.2f}")
        print(f"Temperature: mean={temp_mean:.2f}, std={temp_std:.2f}")
        print(f"Wind speed: mean={wind_mean:.2f}, std={wind_std:.2f}")
        
        return normalized_flow, normalized_features, stats
    
    def create_samples(self, flow_data, external_features, t_params=(12, 3, 3)):
        """
        Create training samples with closeness, period, and trend components.
        
        Args:
            flow_data: Normalized flow [n_timeslots, 2, 32, 32]
            external_features: Normalized features [n_timeslots, 21]
            t_params: (len_closeness, len_period, len_trend)
            
        Returns:
            X_closeness: [n_samples, len_closeness, 2, 32, 32]
            X_period: [n_samples, len_period, 2, 32, 32]
            X_trend: [n_samples, len_trend, 2, 32, 32]
            X_external: [n_samples, 21]
            Y: [n_samples, 2, 32, 32]
        """
        print(f"Creating samples with t_params={t_params}...")
        
        len_closeness, len_period, len_trend = t_params
        n_timeslots = len(flow_data)
        
        # Period: 1 day ago = 48 slots * 1 = 48
        # Trend: 1 week ago = 48 slots * 7 = 336
        period_interval = 48
        trend_interval = 48 * 7
        
        # Calculate valid sample range
        max_lookback = max(
            len_closeness,
            len_period * period_interval,
            len_trend * trend_interval
        )
        
        X_closeness_list = []
        X_period_list = []
        X_trend_list = []
        X_external_list = []
        Y_list = []
        
        for i in range(max_lookback, n_timeslots):
            # Closeness: recent consecutive slots
            closeness = flow_data[i - len_closeness:i]  # [len_closeness, 2, 32, 32]
            
            # Period: same time slots from previous days
            period_indices = [i - period_interval * (j + 1) for j in range(len_period)]
            period_indices.reverse()  # Chronological order
            period = flow_data[period_indices]  # [len_period, 2, 32, 32]
            
            # Trend: same time slots from previous weeks
            trend_indices = [i - trend_interval * (j + 1) for j in range(len_trend)]
            trend_indices.reverse()
            trend = flow_data[trend_indices]  # [len_trend, 2, 32, 32]
            
            # External features at target time
            external = external_features[i]  # [21]
            
            # Target: current timeslot
            y = flow_data[i]  # [2, 32, 32]
            
            X_closeness_list.append(closeness)
            X_period_list.append(period)
            X_trend_list.append(trend)
            X_external_list.append(external)
            Y_list.append(y)
        
        X_closeness = np.array(X_closeness_list)
        X_period = np.array(X_period_list)
        X_trend = np.array(X_trend_list)
        X_external = np.array(X_external_list)
        Y = np.array(Y_list)
        
        print(f"Created {len(Y)} samples")
        print(f"  X_closeness shape: {X_closeness.shape}")
        print(f"  X_period shape: {X_period.shape}")
        print(f"  X_trend shape: {X_trend.shape}")
        print(f"  X_external shape: {X_external.shape}")
        print(f"  Y shape: {Y.shape}")
        
        return X_closeness, X_period, X_trend, X_external, Y
    
    def split_data(self, *arrays, train_ratio=0.8, val_ratio=0.1):
        """
        Split data into train/val/test sets.
        
        Args:
            *arrays: Arrays to split (must have same length)
            train_ratio: Fraction for training
            val_ratio: Fraction for validation
            
        Returns:
            List of tuples (train, val, test) for each input array
        """
        n_samples = len(arrays[0])
        n_train = int(n_samples * train_ratio)
        n_val = int(n_samples * val_ratio)
        
        print(f"\nSplitting {n_samples} samples:")
        print(f"  Train: {n_train} ({train_ratio*100:.0f}%)")
        print(f"  Val: {n_val} ({val_ratio*100:.0f}%)")
        print(f"  Test: {n_samples - n_train - n_val} ({(1-train_ratio-val_ratio)*100:.0f}%)")
        
        splits = []
        for arr in arrays:
            train = arr[:n_train]
            val = arr[n_train:n_train + n_val]
            test = arr[n_train + n_val:]
            splits.append((train, val, test))
        
        return splits
    
    def save_processed_data(self, data_dict, filename):
        """Save preprocessed data to file."""
        filepath = os.path.join(self.output_dir, filename)
        print(f"Saving processed data to {filepath}...")
        
        np.savez_compressed(filepath, **data_dict)
        print("Saved successfully!")
    
    def process(self, years=["16"], t_params=(12, 3, 3)):
        """
        Main preprocessing pipeline.
        
        Args:
            years: TaxiBJ year(s) to process (e.g., ["16"] or ["13", "14", "15", "16"])
            t_params: Temporal parameters (closeness, period, trend)
        """
        if isinstance(years, str):
            years = [years]
        
        years_str = ",".join(years)
        
        print("="*60)
        print("TaxiBJ Data Preprocessing Pipeline")
        print("="*60)
        print(f"Processing years: {years_str}")
        print()
        
        # Step 1: Load flow data
        flow_data, flow_dates = self.load_flow_data(years)
        
        # Step 2: Filter timeslots (6am-11pm)
        flow_data, flow_dates, valid_indices = self.filter_timeslots(flow_data, flow_dates)
        
        # Step 3: Load external data
        meteo_data, meteo_dates = self.load_meteorology()
        holidays = self.load_holidays()
        
        # Step 4: Align external features
        external_features = self.align_external_features(
            flow_dates, meteo_data, meteo_dates, holidays
        )
        
        # Step 5: Normalize data
        flow_data_norm, external_features_norm, stats = self.normalize_data(
            flow_data, external_features
        )
        
        # Step 6: Create samples
        X_c, X_p, X_t, X_ext, Y = self.create_samples(
            flow_data_norm, external_features_norm, t_params
        )
        
        # Step 7: Split train/val/test
        splits = self.split_data(X_c, X_p, X_t, X_ext, Y)
        (X_c_train, X_c_val, X_c_test) = splits[0]
        (X_p_train, X_p_val, X_p_test) = splits[1]
        (X_t_train, X_t_val, X_t_test) = splits[2]
        (X_ext_train, X_ext_val, X_ext_test) = splits[3]
        (Y_train, Y_val, Y_test) = splits[4]
        
        # Step 8: Save processed data
        # Create filename suffix based on years
        if len(years) == 1:
            suffix = years[0]
        else:
            suffix = f"{years[0]}-{years[-1]}"
        
        train_data = {
            "X_closeness": X_c_train,
            "X_period": X_p_train,
            "X_trend": X_t_train,
            "X_external": X_ext_train,
            "Y": Y_train
        }
        self.save_processed_data(train_data, f"BJ{suffix}_train.npz")
        
        val_data = {
            "X_closeness": X_c_val,
            "X_period": X_p_val,
            "X_trend": X_t_val,
            "X_external": X_ext_val,
            "Y": Y_val
        }
        self.save_processed_data(val_data, f"BJ{suffix}_val.npz")
        
        test_data = {
            "X_closeness": X_c_test,
            "X_period": X_p_test,
            "X_trend": X_t_test,
            "X_external": X_ext_test,
            "Y": Y_test
        }
        self.save_processed_data(test_data, f"BJ{suffix}_test.npz")
        
        # Save normalization stats
        stats_file = os.path.join(self.output_dir, f"BJ{suffix}_stats.pkl")
        with open(stats_file, "wb") as f:
            pickle.dump(stats, f)
        print(f"Saved normalization stats to {stats_file}")
        
        print("\n" + "="*60)
        print("Preprocessing complete!")
        print("="*60)


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(
        description="Preprocess TaxiBJ dataset",
        epilog="""
Examples:
  # Process single year
  python src/preprocess.py --years 16
  
  # Process multiple years
  python src/preprocess.py --years 13,14,15,16
  
  # Process range of years
  python src/preprocess.py --years 13-16
        """
    )
    parser.add_argument("--years", type=str, default="16", 
                        help="Year(s) to process: single (16), comma-separated (13,14,15,16), or range (13-16)")
    parser.add_argument("--data_dir", type=str, default="data",
                        help="Directory containing raw data files")
    parser.add_argument("--output_dir", type=str, default="data/processed",
                        help="Directory to save processed data")
    parser.add_argument("--len_closeness", type=int, default=12,
                        help="Number of closeness steps")
    parser.add_argument("--len_period", type=int, default=3,
                        help="Number of period steps")
    parser.add_argument("--len_trend", type=int, default=3,
                        help="Number of trend steps")
    
    args = parser.parse_args()
    
    # Parse years argument
    years_str = args.years
    if "-" in years_str and "," not in years_str:
        # Range format: 13-16
        start, end = years_str.split("-")
        years = [str(y).zfill(2) for y in range(int(start), int(end) + 1)]
    elif "," in years_str:
        # Comma-separated: 13,14,15,16
        years = [y.strip().zfill(2) for y in years_str.split(",")]
    else:
        # Single year: 16
        years = [years_str.zfill(2)]
    
    preprocessor = TaxiBJPreprocessor(
        data_dir=args.data_dir,
        output_dir=args.output_dir
    )
    
    t_params = (args.len_closeness, args.len_period, args.len_trend)
    preprocessor.process(years=years, t_params=t_params)
